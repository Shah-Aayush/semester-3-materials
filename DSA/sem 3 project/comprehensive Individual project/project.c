...
//Print : utility function to print Inorder traversal of the tree.
...
int main() {
	...
	srand(time(0));		//it will help to generate unique sequence of random numbers every time you compile.
	for(i=0;i<52;i++){
	...
	for(i=0;i<NumberOfCardsUsedInGame;i++){
		struct Node *tempNode = (struct Node *)malloc(struct Node);
		tempNode = insertNode(&root[count%players], list[i]);
		root[i%players] = *insertNode(&root[count%players], list[i]);		//assiging it as a card to each player.
		printf("i:%d | number : %d | Assigined to %d\n",i,list[i],i%players);
	}
