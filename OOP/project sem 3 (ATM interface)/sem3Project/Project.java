...
public class Project
{
	public static class database
	{
		...
		void withdraw(long acc_num) throws IOException
		{
			...
			if(!f.exists()){			//if there is no customerDatabase is available
			...
			String line = readAdminDatabase.readLine();		//going to first id
			while(line!=null){
				...
				if(line.equals(Long.toString(acc_num))){
					...
					line = readAdminDatabase.readLine();		//going to PIN
					
					if(line.equals(Short.toString(pin))){
						line = readAdminDatabase.readLine();		//going to balance 
					...
					for(int i=0;i<10;i++){
						line = readAdminDatabase.readLine();		//will skip the password part
					}
				}
				line = readAdminDatabase.readLine();	//skipping *
				line = readAdminDatabase.readLine();	//going to next id
			}
			
				//updating record STARTS
				if(flag==1){
					...
					StringBuffer sb = new StringBuffer();
						...
						//for reading
//						readAdminDatabase = new BufferedReader(new FileReader("./Customer/customerDatabase.txt"));
						line = br.readLine();		//going to first id
						sb.append(line);
						sb.append("\n");
						
					while(line!=null){
						if(line.equals(Long.toString(acc_num))){
							line = br.readLine();	//reading PIN
							sb.append(line);
							sb.append("\n");
							line = br.readLine();	//ignoring the older balance
							sb.append(Long.toString(currentBalance));
							sb.append("\n");
							for(int i=0;i<8;i++){
								line = br.readLine();		//will cover that id's remainig part
								if(line!=null){
									sb.append(line);
									sb.append("\n");
								}
						...
						line = br.readLine();	//skipping *
						if(line!=null){
							sb.append(line);
							sb.append("\n");
						}
						line = br.readLine();	//going to next id
						if(line != null){
							sb.append(line);
							sb.append("\n");
						}	
					...
					fr.close();
					FileWriter fw = new FileWriter("./Customer/customerDatabase.txt",false);	//not appending but erasing data and then writing new Data
		...
		public void delete(long acc_num) throws IOException
		{
			...
			if(!customerDatabase.exists()){			//if there is no customerDatabase is available
				System.out.println("# Customer database doesn't exists! #");
			}
			...
			int flag = 0;
			while(line!=null){
				if(line.equals(Long.toString(acc_num))){			//if account found then no need to enter pin for banker/admin
						
						//deleting record STARTS
						File f1 = new File("./Customer/customerDatabase.txt");
						FileReader fr= new FileReader(f1);
						BufferedReader br = new BufferedReader(fr);
						StringBuffer sb = new StringBuffer();
						while((line = br.readLine())!=null){
							if(line.equals(Long.toString(acc_num))){
								...
								line = br.readLine();		//skipping the whole id part
								continue;		//skipping to append in sb
							}
							else {
								sb.append(line);		//appending num
								sb.append("\n");
								for(int counter=0;counter<11;counter++){
									line = br.readLine();
									sb.append(line);
					...
					line = readCustomerDatabase.readLine();		//will skip the wholeid part
				}
				line = readCustomerDatabase.readLine();	//skipping *
				line = readCustomerDatabase.readLine();	//going to next id
...
//				System.out.println("read id : " + line);
		...
		void deleteAdmin() throws IOException{
			...
			//FILE HANDLING STARTS
				...
				if(!adminDatabase.exists()){			//if there is no adminDatabase is available then it will create default admin id and admin password
				...
				while(line!=null){
					if(line.equals(admin_id)){
						...
						if(line.equals(admin_pass)){
							...
							while((line = br.readLine())!=null){
								if(line.equals(admin_id)){
									line = br.readLine();		//skipping password
									line = br.readLine();		//skipping *
									continue;		//skipping to append in sb
								}
								sb.append(line);
							...
							fr.close();
							FileWriter fw = new FileWriter("./Admin/adminDatabase.txt",false);	//not appending but erasing data and then writing new Data
							fw.write(sb.toString());
							fw.close();
							br.close();
...
//						sc.next();
					...
					case 4:		//create admin acc
						createAdmin();
						break;
					
					case 5:		//delete admin acc
						deleteAdmin();
						break;
